...
void Addr1XMultiTool::shiftBufferOnce(uint8_t direction, uint16_t length) {
#ifdef USE_I2C_REV5_INSTEAD_OF_WIRE
	uint8_t params[3];
	params[0] = 1; // hardcode the header number... this is only a 1X addressable shield
	params[1] = direction;
...
void Addr1XMultiTool::drawFrame(uint16_t pixelCount, uint8_t pixelRGBs[]) {
...
void TurBullEncabulator::upUpDownDownLeftRightLeftRightBA() {
	...
	I2c.setSpeed(1);
#else
 	Wire.begin(I2C_MASTER, 0, I2C_PINS_16_17, I2C_PULLUP_EXT, I2C_RATE_1200);
#endif
